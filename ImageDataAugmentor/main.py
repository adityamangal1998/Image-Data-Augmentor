...
from ImageDataAugmentor import utils, core
...
class ImageDataAugmentation:
    ...
    def scale_augmentor(self, scale_factor=None):
        if isinstance(self.original_images,list):
            augmented_image_list = []
            for image_file in self.original_images:
                augmented_image_list.append([core.scale(image=image_file, scale_factor=scale_factor),'scale_augmentor'])
            return augmented_image_list
        else:
            return self.original_images

    def saturation_augmentor(self,saturation_factor=1.5):
        if isinstance(self.original_images,list):
            augmented_image_list = []
            for image_file in self.original_images:
                augmented_image_list.append([core.saturation(image=image_file, saturation_factor=saturation_factor),'saturation_augmentor'])
            return augmented_image_list
        else:
            return self.original_images

    def brightness_augmentor(self,brightness_factor=1.5):
        if isinstance(self.original_images,list):
            augmented_image_list = []
            for image_file in self.original_images:
                augmented_image_list.append([core.brightness(image=image_file, brightness_factor=brightness_factor),'brightness_augmentor'])
            return augmented_image_list
        else:
            return self.original_images

    def blur_augmentor(self):
        self.augmented_images.extend(core.blur())

    def contrast_augmentor(self):
        self.augmented_images.extend(core.contrast())

    def sharpen_augmentor(self):
        self.augmented_images.extend(core.sharpen())

    def salt_and_paper_augmentor(self):
